...
const ChronoBankPlatformTestable = artifacts.require("./CAVPlatformTestable.sol");
...
contract('ChronoBankPlatform', function(accounts) {
  ...
  before('setup', function(done) {
    ChronoBankPlatformTestable.deployed()
        .then(instance => chronoBankPlatform = instance)
        .then(() => {
            return chronoBankPlatform.setupEventsHistory(chronoBankPlatform.address);
        })
        .then(() => {
...
context("with one CBE key", function(){
  ...
  it('should not be possible to trust to already trusted address', function() {
    ...
    return chronoBankPlatform.trust(trustee).then(function() {
      ...
      assert.equal(result, ErrorsEnum.PLATFORM_ALREADY_TRUSTED);
    });
  });
  it('should not be possible to trust to oneself', function() {
    var holder = accounts[0];
    return chronoBankPlatform.trust.call(holder).then(function(result) {
      assert.equal(result, ErrorsEnum.PLATFORM_CANNOT_APPLY_TO_ONESELF);
    });
  });
  ...
  it('should not be possible to recover to existing holder', function() {
    ...
    return chronoBankPlatform.trust(trustee).then(function() {
      ...
      assert.equal(result, ErrorsEnum.PLATFORM_SHOULD_RECOVER_TO_NEW_ADDRESS);
    });
  });
